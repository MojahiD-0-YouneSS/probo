from mui.request.props import RequestProps
...
class RequestDataTransformer:
    ...
    example:
        ...
        >>>             rdt.save_forms()
    """
    def __init__(self, request=None, form_class=None, request_files=False, mono_form=True):
        self.target_data = {}
        ...
        if mono_form:
            self.form = form_class(request.POST,request.FILES) if form_class and request_files else (form_class(request.POST) if form_class else None)
        else:
            self.form = self._initialize_multi_form_processing()
            
        self.user_data = self._prepare_user_data() if hasattr(request,'user') else dict()
        self.request_files = self._process_post_files() if hasattr(request,'FILES') else dict()
        self.post_data = self._process_post_data() if hasattr(request,'POST') else dict()
        self.get_data = self._process_get_data() if hasattr(request,'GET') else dict()
        self.session_data = self._get_all_session_data() if hasattr(request,'session') else dict()
    ...
    def _process_get_data(self):
        ...
        for field, field_data in self.request.GET.items():
            if isinstance(field_data, list):
                if len(field_data) == 1:
                    get_data_dict[field] = field_data[0]  # Single value, store as string
                else:
    ...
    def _process_post_data(self):
        ...
        for field, field_data in self.request.POST.items():
            if isinstance(field_data, list):
                if len(field_data) == 1:
                    post_data_dict[field] = field_data[0]  # Single value, store as string
                else:
    ...
    def get_request_data(self, organized=False):
        ...
        } if organized else {**self.cleaned_data, **self.post_data, **self.get_data, **self.request_files}
    ...
    def extract_target_data(self, fields=None, multi_value_fields=None, is_json=False):
        ...
        """
        if is_json:
            self.target_data = self.request.json() if hasattr(self.request, "json") else {}
        else:
    ...
    def has_field(self, field):
        ...
        """
        return field in self.post_data or (hasattr(self.request, "json") and field in self.request.json())
    ...
    def extract_props(self):
        ...
        global_ctx = {
            ...
            'is_authenticated': self.request.user.is_authenticated if self.request else False,
...
class FormHandler:
    ...
    def mono_form_true_option(self,):
    ...
    def mono_form_false_option(self,):
