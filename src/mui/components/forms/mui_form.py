from mui.components.component import (
    Component,
)
from mui.components.state.props import StateProps
from mui.components.state.component_state import (
    ComponentState,ElementState,
)
from mui.styles.elements import (
    ComponentStyle,
)
...
from mui.request.transformer import (
    FormHandler,
...
class MastodonFormField:
    """
    A declarative renderer for individual form fields.
    Bridging Django BoundFields with MUI Elements.

    This class handles the extraction of widget attributes (type, class, choices)
    from Django fields and renders them into semantic HTML using MUI's functional tags.
    ...
    Example:
        >>> # Automatic (Django)
        >>> mff = MastodonFormField(dj_field=form['username'])
        >>>
        >>> # Manual
        >>> mff = MastodonFormField(
        ...     tag_name='input', 
        ...     type='email', 
        ...     name='email', 
    ...
    def __init__(self,
        ...
        **attrs:dict[str,str],
        ...
        if tag_name:
            info = self._make_info(self.attrs,self.field_label,{'for': attrs.get('id',None)},content)
            self._field_build(tag_name,**info)
    
    def _make_info(self,attrs:dict[str,str],label_string:str,label_attr:dict[str,str],content:str=None)-> dict[str,str]:
        return {
    ...
    def _field_build(self,tag:str,**info):
        ...
        if valid_tag:
            ...
            tag_string = tag_method(**tag_attrs) if tag_void else tag_method(tag_content, **tag_attrs)
            if not label_string:
                ...
                self.widget_info[f'{input_id}-{tag}'] =label(label_string,**label_attrs)+ tag_string
        return self
    ...
    def include_dj_field(self,dj_field):
        if hasattr(dj_field, 'field') and hasattr(dj_field.field, 'widget'):
            ...
            info = self._make_info(dj_attrs,dj_label,  {'for': dj_attrs.get('id',None)},)
    ...
    def add_input(self,  label_string:str=None,label_attrs:dict[str,str]=None,**input_attrs):
        '''
        <label for="xyz">...</label>
        <input type="..." id="xyz" name="xyz">
        if 
        '''
        info =  self._make_info(input_attrs,label_string,label_attrs,)

        return self._field_build('input',**info) 
    
    def add_textarea(self,textarea_content:str=None,  label_string:str=None,label_attrs:dict[str,str]=None,**textarea_attrs):
        
        _defaults = {'textarea_rows':8, 'textarea_cols':50,}
    ...
    def add_select_option(self, option_values:list[str],selected_options_indexes:list[int]=None,label_string:str=None,label_attrs:dict[str,str]=None,**select_attrs):
        '''option '''
        content =' '.join([
            option(y,value=y,selected=True) if x in selected_options_indexes else option(y,value=y) for x,y in enumerate(option_values)])
       
        info = self._make_info(select_attrs,label_string,label_attrs,content)
        
        return self._field_build('select',**info)
    
    def add_select_optgroup(self, optgroups:dict[str,list[str]], selected_options_indexes:list[int]=None, label_string:str=None, label_attrs:dict[str,str]=None, **select_attrs):
        '''option '''
        
        optgroup_content =[]
        for x,y in enumerate(optgroups.values()):
            optgroup_content.append(''.join([option(v,value=v,selected=True) if x in selected_options_indexes else option(v,value=v) for v in y]) )
        content = ''.join([optgroup(k,**{'label':v})  for k,v in zip(optgroup_content,optgroups.keys())])
        info =self._make_info(select_attrs,label_string,label_attrs,content) 
        
        return self._field_build('select',**info)
    
    def add_field_set(self, legend_content:str, form_elements:list[str], label_string:str=None,label_attrs:str=None,  **fieldset_attrs):
    ...
    def add_data_list(self, option_value_list:list[str], label_string:str=None,label_attrs:dict[str,str]=None, **data_list_attrs):
    ...
    def add_output(self, label_string:str=None,label_attrs:dict[str,str]=None, **output_attrs):
    ...
    def render(self,) -> str:
        ...
        if self.dj_field_info:
            ...
            if tag:
                err_list = [div(str(e), Class='invalid-feedback') for e in self.dj_field_info[tag]['errors']]
                errors_html = div(*err_list, class_='errors')
                state_error = {
                    'field_errors':errors_html,
                }
        errors = ElementState('div',s_state='field_errors')
        error_string = errors.change_state(state_error,).state_placeholder or str()
...
class MastodonForm:
    ...
    and MUI's frontend rendering. It supports both automatic rendering of Django forms
    and manual construction of forms for static sites or other frameworks.

    Args:
        action (str): The URL to submit the form to (e.g., "/login/").
        *form_field_declarations (tuple[MastodonFormField]): Variable length list of manual field declarations.
    ...
    Example:
        ...
        >>> form = MastodonForm(action=".", request_data=rdt)
        >>> html = form.render()
        >>> # Manual Mode
        >>> email_field = MastodonFormField("input", 'email to contact with',Type="email",value="",name="email",)
        >>> form = MastodonForm("/search",email_field, method="get", manual=True, csrf_token="xyz")
        >>> html = form.render()
    ...
    def __init__(self,
        action:str, 
        *form_field_declarations:tuple[MastodonFormField],
        ...
        if not manual and self.request_data: 
            self.form_class = self.request_data.form_class if not self.form_class  else self.form_class 
            if self.request_data.is_valid():
               self.is_valid= self.validate()

    def get_csrf_token(self):
        if self._manual_csrf: return self._manual_csrf
        if self.request_data: return self.request_data.get_csrf_token()
        return ""
    ...
    def render(self,):
        ...
        if self.request_data and self.request_data.form:
            ...
            for field in self.request_data.form:
                ...
                mff = MastodonFormField(dj_field=field)
