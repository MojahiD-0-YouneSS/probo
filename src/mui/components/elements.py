...
from mui.utility import render_attributes

MARKER = chr(31)
CONTENT_MARKER = f'@mui:{MARKER}'

class Element:
    """A class to create HTML elements with attributes and content."""
    
    def __init__(self, tag=str(),content: str = '', is_list: bool = False, is_natural: bool = False, **attrs):
    ...
    def _element_parser(self, *args, **kwargs):
        ...
        for arg in args:
            if isinstance(arg, str) and Tag.get(arg) and not tag:
                try:
                    ...
                    if result:
                        tag=result
                        element_obj= self.__class__(is_list=self.is_list, is_natural= self.is_natural)
        ...
        return {'tag':tag,'attrs':attrs_dict,'content':content,}
    ...
    def element_health(self,opening_tag:str):
        ...
        attribute_value = ElementAttributeValidator(element_tag=opening_tag, **self.attrs)
    ...
    def raw(self, *string ,inner=False,is_comment=False) -> 'Element':
        ...
        if self.is_list:
            self.element.extend(['<!--', *string, '-->']) if is_comment else self.element.extend(list(string))
        else:
            self.element += '<!--'+STRING+'-->' if is_comment else STRING
        return self

    def set_data(self, *string) -> 'Element':
        self.content += ' '.join([f'<$mui-var name="{str(string_arg)}"/>' for string_arg in string])
        return self
            
    def custom_element(self,cstm_tag,content='',is_void_element=False,**attrs):
        tag = Enum('tag', {
            cstm_tag.upper(): [cstm_tag.lower(), {"void": True if cstm_tag.lower() in VoidTags.VOID_TAGS.value or is_void_element else False}]
        })
        if tag or attrs:
...
class Head:
    ...
    def add(self, element, key=None):
        ...
        self._registry[key] = element.element if isinstance(element,Element) else str(element)
        return self
    ...
    def render(self, *extra_head_content):
        ...
        for x in extra_head_content:
            self.add(x)
        head_tag = Element().set_content("".join([el for el in self._registry.values()])).head().element
...
class Template:
    ...
    def __init__(self, separator: str = "\n", **components):
        ...
        self.head.register_meta(name="viewport", content="width=device-width, initial-scale=1.0")
        self.head.set_title("MUI Page")
        self.__loaded_base=''
    ...
    def render(self) -> str:
        ...
        document = Element().doctype().element + Element().set_attrs(lang="en").set_content(
