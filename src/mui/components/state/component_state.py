...
from mui.components.attributes import ElementAttributeValidator
import re
...
class ComponentState:
    ...
    Example:
        ...
        ... print(state.resolve_template(es.placeholder))
            <h1>Live Update</h1>
    """
    def __init__(self,*elements_states:tuple['ElementState'],s_data:dict[str,Any]=None, d_data:dict[str,Any]=None,incoming_props:dict[str,Any]=None,strict:bool=False, **props:dict[str,Any]) -> None:
    ...
    def _determine_state(self, s_el,d_el)->tuple[dict[str,str]]:
        ...
        if self.strict:
            if not d_value and not s_value :
                raise KeyError(f"[MUI Strict] key  missing in d_data (and no static fallback).")
    ...
    def validate_global_props(self) -> bool:
        ...
        if not self.incoming_props:
            ...
            if self.strict:
                raise ValueError(f"[MUI Strict] {self.state_errors}")
            return False
    ...
    def use_state(self,) -> Self:
        ...
        if  self.state_errors and self.strict:
            raise ValueError(f"[MUI Strict]⚠️ Rendering Blocked: {self.state_errors}")
        ...
        for k, el in self.resolved_state_elements.items():
            if el.placeholder in template:
                if el.state_placeholder is None or self.state_errors:
                    ...
                    template = re.sub(re.escape(el.placeholder), el.state_placeholder, template)
...
class ElementState:
    ...
    """
    def __init__(self,element, s_state:str=str(), d_state:str=None,c_state=str(), is_custom=False,props:StateProps=None,bind_to=None, inner_html=None,i_state=False,hide_dynamic=False,is_void_element: bool = False,**attrs):
        import uuid

        self.state_id=f'{element}=={uuid.uuid4().hex}'
        self.placeholder = Element().custom_element('$',content=Element().custom_element(element,s_state,is_void_element,**attrs).element,s=s_state,d=d_state,c=c_state,i=bool(i_state)).element#<$ s="..." d='None' i='None'>...</$>
        ...
        self.attrs={k.lower().replace('_','-'):v for k,v in attrs.items()} if attrs else {}
        ...
        self.inner_html = inner_html if callable(inner_html) else  lambda x:str(x) if not isinstance(x,tuple) else str(x[1])
        if not self.is_custom:
            # Check if static attrs passed in __init__ are valid
            self.valid_element = ElementAttributeValidator(f'<{self.element}>', **self.attrs).is_valid
        else:
    ...
    def change_state(self,data:dict[str,str],props=dict()):
        ...
        if not self.props.validator(props).is_valid() or not self.valid_element:
            ...
            if not self.is_void_element:
                if self.bind_to and ( not self.c_state or not self.bind_to in self.attrs):
                    self.state_placeholder=None
                else:
                    if self.hide_dynamic and self.d_state and not self.d_state in data:
                        self.state_placeholder=None
                    elif self.d_state and self.d_state in data:
                        if self.i_state:
                            self.state_placeholder=''.join([Element().custom_element(self.element,self.inner_html(d),**self.attrs).element for d in enumerate(data.get(self.d_state))])
                        else:
                            self.state_placeholder=Element().custom_element(self.element,self.inner_html(self.c_state),**self.bind_data_to(str(data.get(self.d_state)))).element if self.bind_to else Element().custom_element(self.element,self.inner_html(data.get(self.d_state)),**self.attrs).element
                    elif self.s_state in data:
                        if  self.i_state:
                            self.state_placeholder=''.join([Element().custom_element(self.element,self.inner_html(s),**self.attrs).element for s in enumerate(data.get(self.s_state))])
                        else:
                            self.state_placeholder=Element().custom_element(self.element,self.inner_html(self.c_state),**self.bind_data_to(str(data.get(self.s_state)))).element if self.bind_to else Element().custom_element(self.element,self.inner_html(data.get(self.s_state)),**self.attrs).element
                    else:
                        self.state_placeholder=None
            else:
                target_value = data.get(self.s_state) or data.get(self.d_state)
                self.state_placeholder=Element().custom_element(self.element,**self.bind_data_to(str(target_value))).element if self.bind_to else Element().custom_element(self.element,is_void_element=self.is_void_element,**self.attrs).element
        return self
