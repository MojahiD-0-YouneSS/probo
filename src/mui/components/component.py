...
from mui.styles.elements import (
    ComponentStyle, 
    element_style_state,
    CssRule,CssSelector,SelectorRuleBridge,
)
...
class Component:
    ...
    """
    The base class for all UI components in MUI.
    ...
    def __init__(self, name:str, state:ComponentState=None, template:str=str(),props:dict=None,  **elements):
        self.name: str = name
        self.index: int = 0
        
        self.children: dict[str,Any] = elements or {}
        self.children_info = {k:TemplateResolver(v).template_resolver() for k,v in elements.items()}
        
        if template:
            self.template_obj = TemplateResolver(tmplt_str=template, load_it=True)
        else:
            self.template_obj = None #TemplateResolver(tmplt_str=template, load_it=True)
    ...
    @classmethod
    def register(cls, name:str, state:ComponentState=None, props:dict=None, *elements):
    ...
    def add_child(self, child:'str|Component', name:str=None):
        ...
        if child:
            if isinstance(child,type(self)):
                ...
                child_obj = TemplateResolver(tmplt_str=child, load_it=True).template_resolver()
    ...
    def render(self, override_props:dict=None,force_state:bool=False,add_to_global:bool=False) -> str|tuple:
        """
        ...
        if self.is_root_element:
            final_template = Element(
                ...
                **self.root_element_attrs
            ).build_tag(self.root_element_tag,)
        if self.active_css_rules:
            valid_css = element_style_state(final_template,self.comp_state.resolved_state_elements, *self.active_css_rules)
            self.cmp_style = ComponentStyle(final_template, *valid_css)
            return  final_template, self.cmp_style.render()
        else:
            return final_template

    def change_skin(self, source:dict['str',Any]|Self=None, root_attr:str=None,root_attr_value:str=None, **root_css:dict['CssSelector','CssRule']):
        """
        ...
        elif hasattr(source, 'active_css_rules') and isinstance(source.active_css_rules, list):
        ...
        if root_css:
            if root_attr:
                if self.root_element_attrs and root_attr not in self.root_element_attrs:
                    self.root_element_attrs[root_attr] = ('true' if not root_attr_value else root_attr_value)            
