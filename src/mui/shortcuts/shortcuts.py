...
from mui.components.state.props import (
    StateProps,
) # 1
from mui.components.component import (
    Component,
) # 1
from mui.components.elements import (
    Element,
    Head,
    Template,
) # 3
from mui.components.attributes import (
...
from mui.context.django import (
    DjangoComponent,
) # 1
...
from mui.styles.frameworks.bs5.custom import (
    BS5Customization,
) # 1
...
from mui.styles.plain_css import (
    CssRule,
    CssSelector,
    Animation,
    MediaQueries,
...
from mui.styles.utils import (
    resolve_complex_selector,
    selector_type_identifier,
) #2
from mui.templates.default_templates import (
    base_template,
) # 1
from mui.templates.resolver import (
    TemplateResolver,
) # 1
from mui.terminal.emmet import (
    emmet,
) # 1
...
from mui.shortcuts.shortcuts_utils import (    
    make_es_from_esc,
)
from mui.xml.xml import (
    HtmlToXmlConverter,
) # 1
from mui.utility import (
    exists_in_dict,
    render_attributes,
) #2 
from typing import (
...
def custom(tag: str, content: str = '',is_void_element: bool = False, **attrs) -> str:
    ...
    return Element().custom_element(tag, content=content, is_void_element=is_void_element, **attrs).stringify_element().element
 
def set_data(*variables) -> str:
    """Creates a custom HTML tag string immediately."""
    return Element().set_data(*variables).stringify_element().element
 
def form_field(tag, **kwargs) -> MastodonFormField:
    return MastodonFormField(tag_name=tag, **kwargs)
...
def mastodon_form(config: FormConfig) -> str:
    ...
    if config.request and config.form_class:
        ...
        mf = MastodonForm(config.action, request_data=rdt, method=config.method)
    else:
        mf = MastodonForm(config.action, *config.fields, method=config.method, manual=True, csrf_token=config.csrf_token)
    return mf.render()
...
def head_seo(config: HeadConfig) -> Head:
    ...
    if config.seo:
        ...
        # Social (Open Graph)
        if s.og_title: head.register_meta(property="og:title", content=s.og_title)
        if s.og_type: head.register_meta(property="og:type", content=s.og_type)
        if s.og_image: head.register_meta(property="og:image", content=s.og_image)
        
        # Twitter
        if s.twitter_card: head.register_meta(name="twitter:card", content=s.twitter_card)
        if s.twitter_creator: head.register_meta(name="twitter:creator", content=s.twitter_creator)
