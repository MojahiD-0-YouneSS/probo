...
class Lists(Enum):
    ...
    LIST_GROUP_FLUSH = "list-group-flush"  # Flush list group (no borders and rounded corners)

    LIST_GROUP_HORIZONTAL = "list-group-horizontal"  # Horizontal list group (default breakpoint)
    LIST_GROUP_HORIZONTAL_SM = "list-group-horizontal-sm"  # Horizontal on small devices
    LIST_GROUP_HORIZONTAL_MD = "list-group-horizontal-md"  # Horizontal on medium devices
    LIST_GROUP_HORIZONTAL_LG = "list-group-horizontal-lg"  # Horizontal on large devices
    LIST_GROUP_HORIZONTAL_XL = "list-group-horizontal-xl"  # Horizontal on extra large devices
...
class Cards(Enum):
    ...
    CARD_COLUMNS = "card-columns"  # Deprecated in Bootstrap 5, was used for card columns
...
class Collapse(Enum):
    ...
    ACCORDION_BUTTON_COLLAPSED = "accordion-button collapsed"  # Collapsed state of accordion button
...
class Navbar(Enum):
    ...
    NAVBAR_EXPAND_XXL = "navbar-expand-xxl"  # Expand navbar on extra-extra-large screens and up
...
class Toast(Enum):
    ...
    TOAST_POSITION_TOP_START = "toast-container position-fixed top-0 start-0 p-3"  # Toast container top left
    TOAST_POSITION_TOP_END = "toast-container position-fixed top-0 end-0 p-3"  # Toast container top right
