...
class TemplateResolver:
    ...
    def merge_attributes(self, existing_attrs: dict, new_attrs: dict) -> dict:
        ...
        for key, value in new_attrs.items():
            if key == "class":
                ...
                existing_attrs["class"] = " ".join(existing_classes.union(new_classes)).strip()
            else:
        ...
        def _traverse(elem):
            if elem.tag != "root":
                ...
                if elem.tag not in result:
                    ...
                    result[elem.tag] = self.merge_attributes(result[elem.tag], elem.attrib)
    ...
    def invert_tag_attribute_dict(
        ...
        if unique:
            result = {k: list(dict.fromkeys(v)) for k, v in result.items()}  # preserves order
...
#        self.template_info = result
    ...
    def template_resolver(self,template=None,load_it=False):
        if template:
            ...
            info = self.__template_resolver(tmplt_str=self.tmplt_str,load_it=self.load_it)
