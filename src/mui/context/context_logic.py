...
class TemplateProcessor:
    ...
    def _process_template_block(self, text: str, current_context: dict) -> str:
        """
        Processes only <$if>, <$elif>, <$else>, and <$for> blocks.
        """
    ...
    def _process_if_blocks(self, text: str, context: dict) -> str:
        ...
        def repl(match):
            ...
            try:
                ...
                elif elif_condition and self._evaluate_expression(elif_condition, context):
    ...
    def if_true(expression, if_block, style='mui', else_statement=None, **elif_statements):
        if style == 'mui':
            return f"<$if {expression}>{if_block}" + \
                "".join([f"<$elif {cond}>{block}" for cond, block in elif_statements.items()]) + \
                (f"<$else>{else_statement}" if else_statement else '') + "</$if>"
        elif style == 'django':
            return f"{{% if {expression} %}}{if_block}" + \
                "".join([f"{{% elif {cond} %}}{block}" for cond, block in elif_statements.items()]) + \
                (f"{{% else %}}{else_statement}" if else_statement else '') + "{% endif %}"
        else: return ""
    @staticmethod
...
def loop(data, renderer):
    ...
    results = []
    iterator = []
    if not renderer: renderer = lambda u:u
