import pytest
from mui.shortcuts import (
    custom,
    component,
    layout,
    semantic_layout,
    mastodon_form,form_field,iterator,
    xml,
    theme,
    datatable,
    head_seo,
...
from mui.shortcuts.configs import (
    ElementStateConfig, StateConfig, StyleConfig, ComponentConfig, SemanticLayoutConfig, LayoutConfig,FormConfig,
    ListConfig, XmlConfig, ThemeConfig, TableConfig, 
    HeadConfig, SEOConfig, PageConfig
)
from mui.components.elements import Head, Template, Element
# ==============================================================================
...
def test_flow_component_with_state_data():
    ...
    state_conf = StateConfig(
        s_data={'username': 'Youness'},
        elements_state_config=[esc]
    )
...
def test_flow_component_bootstrap_root():
    ...
    """
    style_conf = StyleConfig(
        root_bs5_classes=['btn', 'btn-primary']
    )
    config = ComponentConfig(
        name="BS5Comp",
...
def test_iterator_basic_list():
    ...
    conf = ListConfig(items=["A", "B"], wrapper_tag="ul", item_renderer=lambda x: f"<li>{x}</li>")
    html = iterator(conf)
...
def test_iterator_nested_elements():
    ...
    items = [div("1"), div("2")]
    conf = ListConfig(items=items, wrapper_tag="section") # Default renderer just converts to str/render
    html = iterator(conf)
