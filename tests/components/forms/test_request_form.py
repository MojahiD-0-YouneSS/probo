import pytest
from mui import MastodonForm, RequestDataTransformer, MastodonFormField

import pytest
from django import forms
from django.test import RequestFactory
from django.core.files.uploadedfile import SimpleUploadedFile
from mui import MastodonForm, RequestDataTransformer
...
class MockField:
    ...
    def __init__(self, name, value="", errors=None, required=False, widget=None):
        ...
        self.field = type('Inner', (), {'widget': widget or MockWidget(), 'required': required})
    
    def value(self): return self._value
    def id_for_label(self): return f"id_{self.name}"
    def label_tag(self): return f"<label>{self.label}</label>"
...
def test_mf_render_real_django_form():
    """
    Scenario: Render the real ContactForm via MastodonForm.
    ...
    rdt = RequestDataTransformer(request=request, form_class=ContactForm)
    mf = MastodonForm('/contact/',request_data=rdt)
