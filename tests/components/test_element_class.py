...
def test_custom_element_creation():
    ...
    el = Element(is_list=True).custom_element('my-widget', content="Content", is_void_element=False)
...
def test_set_data_django_style(builder):
    ...
    assert '<$mui-var name="toggle_modal=True"/>' in html # Boolean True usually becomes just the attr

def test_raw_comment(builder):
    """Test the raw method wrapping content in comments."""
    builder.set_content("Hidden Data")
    builder.raw("Hidden Data 123",'oijoiiotg','joptjgtijopji') # Should wrap in <!-- -->
