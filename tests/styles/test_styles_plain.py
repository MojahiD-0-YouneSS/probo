...
from mui.styles.plain_css import (
    CssRule, CssSelector, Animation, MediaQueries, 
    box_model, make_important, css_comment, css_style, CssAnimatable,
    CssRuleValidator
)
...
def test_css_rule_apply_css_fonts():
    ...
    css = rule.render()
    assert "font-family:Helvetica, Arial, sans-serif;" in css or \
...
def test_media_query_multiple_rules():
    ...
    css = {CssSelector().cls("a").render():r1.render(),
...
def test_media_query_complex_features():
    ...
    mq = MediaQueries("screen", media_values={"min-width":"768px", "orientation":"landscape"})
    css = mq.render()
...
def test_media_query_not_operator():
    """4. Verify 'not' logic."""
    mq = MediaQueries("screen", {"min-width":"500px",},is_not=True)
    assert "@media not screen" in mq.render()

def test_media_query_no_media_type():
    """4. Verify 'not' logic."""
    mq = MediaQueries(None, {"min-width":"500px",}, no_media_type=True)
...
def test_media_query_only_operator():
    ...
    mq = MediaQueries(
        "screen", 
        media_values={"min-width":"500px",},is_only=True
    )
...
def test_media_query_not_only_operator():
    ...
    mq = MediaQueries(
        "screen", 
        media_values={"min-width":"500px",},is_only=True,is_not=True
    )
...
def test_css_style_with_objects():
    ...
    selectors_rules = {CssSelector().cls('pop'): CssRule(color="red", font_size="12px"), CssSelector().el('div'): CssRule(margin_top="10px")}
    style_str = css_style(
